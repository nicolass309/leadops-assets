...
function buildPosts(calendar) {
  ...
  for (let offset = 0; offset < windowDays; offset += 1) {
    ...
    for (const spec of specs) {
      ...
      posts.push({ ...spec, date, day, dueAt: planned.date, shifted: planned.shifted });
    }
  }
...
async function main() {
  ...
  const newStateRows = ['key,network,date,time,day,due_at,status,post_id,asset,error'];
  const runRows = ['key,network,date,time,day,due_at,status,post_id,asset,error'];
  ...
  for (const post of posts) {
    ...
    if (dryRun) {
      console.log(`[DRY] ${key} ${post.dueAt.toISOString()} ${post.videoUrl ? 'video' : 'text'} chars=${post.text.length}`);
      runRows.push([key, post.network, post.date, post.time, post.day, post.dueAt.toISOString(), 'dry-run', '', post.videoUrl || '', ''].map(csv).join(','));
      continue;
    }
    ...
    try {
      ...
      const row = [key, post.network, post.date, post.time, post.day, created.dueAt, created.status, created.id, post.videoUrl || '', ''].map(csv).join(',');
